...
const FAM_PHOTO_COLORS={
  ...
  // family-grouping correction) — they are not yet browsable Crystals 101 tiles
  // in this pass, so this entry exists only so any future tile inherits the
  // established quartz-family token rather than falling back to plain gray.
  // No new color was invented.
...
function renderFamilies(tier){
  ...
  fc.innerHTML=filtered.map(f=>{
    ...
    return`<div class="fam-card" data-family="${escapeAttr(f.n)}" onclick="${clickAction}" title="${cardTitle}"><div class="fam-photo">${photoSlot}</div><div class="fam-body"><div class="fam-name">${f.n}</div>${cnt?`<div class="fam-count">${cnt} stone${cnt===1?'':'s'}</div>`:''}<div class="fam-energy">${f.energy||f.desc}</div>${examples?`<div class="fam-examples">${examples}</div>`:''}</div></div>`;
  }).join('');
}
